...
class PCTFTransform{
	private:
		...
		// std::string _raw_sub_pc = "/velodyne_obstacles";
	...
	: _nhPrivate("~")
{
	...
	/*parameter*/
	_nhPrivate.param("target_frame", _target_frame, std::string("world"));
	std::cout << "_target_frame = " << _target_frame << std::endl;
	/*subscriber*/
	_sub_pc = _nh.subscribe("/velodyne_obstacles", 1, &PCTFTransform::callbackPC, this);
	/*publisher*/
	_pub_pc = _nh.advertise<sensor_msgs::PointCloud2>("velodyne_obstacles/transformed", 1);
}
...
int main(int argc, char** argv)
{
	...
	// ros::NodeHandle pnh_1("~");
  	// std::string sub_pc="/velodyne_obstacles";
  	// pnh_1.getParam("pointcloud", pc_tf_transform._raw_sub_pc);

	// ros::NodeHandle pnh_2("~");
	// std::string target_frame_id="map";
  	// pnh_2.getParam("tfi", _target_frame);
